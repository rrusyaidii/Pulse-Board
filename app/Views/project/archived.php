            ...
            <div class="col-xl-4 col-md-6 mb-4">
              <div class="project-box border rounded-4 p-3 pb-0 h-100">
                ...
                <?php
                  ...
                  $percentage = $stats['total_tasks'] > 0 ? round(($stats['total_completed'] / $stats['total_tasks']) * 100) : 0;
                  $rawProfit = $stats['contractValue'] - $stats['cost'];
                  $adjustedProfit = round($rawProfit * ($percentage / 100));
                  $progressClass = 'bg-danger';
                  if ($percentage > 30 && $percentage < 70) $progressClass = 'bg-warning';
                  if ($percentage >= 70) $progressClass = 'bg-success';

                  // who can edit this project? check user-specific project role from $projectRoles
                  $currentProjectRole = $projectRoles[$project['projectID']] ?? null;
                ?>

                <div class="row details mb-2">
                  <div class="col-6"><span>Total Task</span></div><div class="col-6 text-primary"><?= $stats['total_tasks'] ?></div>
                  <div class="col-6"><span>In Progress</span></div><div class="col-6 text-primary"><?= $stats['in_progress'] ?></div>
                  <div class="col-6"><span>Total Completed</span></div><div class="col-6 text-primary"><?= $stats['total_completed'] ?></div>
                  <div class="col-6"><span>Contract Value</span></div><div class="col-6 text-primary">RM <?= number_format($stats['contractValue'], 0) ?></div>
                  <div class="col-6"><span>Current Profit</span></div><div class="col-6 text-primary">RM <?= number_format($adjustedProfit, 0) ?></div>
                </div>
                ...
                <?php if (session()->get('role') === 'admin' || $currentProjectRole === 'manager'): ?>
                  <a class="btn btn-primary" href="<?= base_url('project/project/edit/'.$project['projectID']) ?>">Edit</a>
                <?php else: ?>
                  <a class="btn btn-secondary" href="<?= base_url('project/project/view/'.$project['projectID']) ?>">View</a>
                <?php endif; ?>
